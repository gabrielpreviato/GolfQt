...
GolfMap::GolfMap(std::vector<GolfWall> walls, std::vector<GolfFloor> floors, std::map<std::string, Material> materials, int width, int height)
    : m_walls(walls), m_floors(floors), m_materials(materials), m_width(width), m_height(height)
{}

GolfMap::GolfMap(const GolfMap& other)
    : m_walls(other.m_walls), m_floors(other.m_floors), m_width(other.m_width), m_height(other.m_height), m_materials(other.m_materials)
{}
...
GolfMap GolfMap::load(std::string path)
{
    ...
    return GolfMap{walls, floors, materials, data["width"], data["height"]};
}
